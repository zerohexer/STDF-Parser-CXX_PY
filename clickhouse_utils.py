...
def _collect_device_info_map(extractor):
    ...
    # Handle both original extractor.data_store and C++ version (dict with measurements key)
...
def _push_landing_records_if_available(connection_params, extractor, batch_size):
    ...
    # Handle both original extractor.data_store and C++ version (dict with landing_records key)
...
def push_measurements_clickhouse(connection_params, measurements, batch_size=100000):
    """
    ULTRA-FAST measurements pushing with clickhouse-driver (native TCP)
    ...
    """
    print(f"[{datetime.now().strftime('%H:%M:%S')}] 🚀 Ultra-fast push (native TCP) with {len(measurements)} measurements")
    start_time = time.time()
    
    # Single persistent connection (native TCP is fast enough)
    from clickhouse_driver import Client
    client = Client(
        host=connection_params.get('host', 'localhost'),
        port=connection_params.get('port', 9000),
    ...
    print("✅ Single persistent native TCP connection created")
    
    # ULTRA-FAST data conversion (minimal operations)
    print("⚡ Converting data with minimal overhead...")
    convert_start = time.time()
    
    # Use datetime object for clickhouse-driver (as required)
    current_time = datetime.now()
    
    # DIRECT conversion to clickhouse-driver format (list of tuples)
    data_tuples = []
    for m in measurements:
        data_tuples.append((
            m['WLD_ID'],
            ...
            int(m['WP_POS_Y']),
            float(m['WPTM_VALUE']),
            current_time,  # datetime object as required by clickhouse-driver
            1 if m['TEST_FLAG'] else 0,
            0,  # Skip expensive segment calculation
            m.get('FILE_HASH', '')
        ))
    ...
    # Single massive insert with native TCP (fastest possible)
    print("⚡ Single massive native TCP insert...")
    print(f"📊 Insert format: {len(data_tuples):,} tuples, sample: {data_tuples[0] if data_tuples else 'None'}")
    insert_start = time.time()
    
    # Simple clickhouse-driver insert
    client.execute(
        "INSERT INTO measurements (wld_id, wtp_id, wp_pos_x, wp_pos_y, wptm_value, wptm_created_date, test_flag, segment, file_hash) VALUES",
        data_tuples
    )
    ...
    print(f"✅ Ultra-fast native TCP push completed!")
