"""
ClickHouse utility functions for STDF data processing using clickhouse-connect
Cross-platform compatible with Windows, macOS, and Linux

Based on STDF_Parser_CH/clickhouse_utils.py but using clickhouse-connect
for better Windows compatibility and C/Cython optimizations
"""

from clickhouse_connect_wrapper import ClickHouseConnectWrapper as Client, ClickHouseConnectConnectionPool, ClickHouseConnectConnectionManager
import threading
...
def optimize_clickhouse_connection(host='localhost', port=8123, database='default', user='default', password=''):
    """
    Create an optimized ClickHouse client connection with enhanced settings for STDF data processing
    Uses clickhouse-connect for Windows compatibility
    ...
    try:
        ...
        client = Client(
            host=host,
            port=port,  # HTTP port 8123 for clickhouse-connect
            database=database,
            user=user,
            password=password,
            settings={
        ...
        # Verify connection (already done in wrapper init)
...
def setup_clickhouse_schema(client):
    ...
    Parameters:
    - client: ClickHouse client (clickhouse-connect)
    """
    try:
        print("🔧 Setting up ClickHouse schema for STDF data...")
...
def create_materialized_views(client):
    ...
    try:
        ...
        print(f"⚠️ Error creating materialized views: {e}")
        # Don't raise the error since some ClickHouse versions might not support all features
...
def _safe_uint_convert(value, field_name="field", clamp_to_uint8=False):
    ...
    try:
        ...
        if int_val < 0:
            print(f"⚠️ Warning: {field_name} value {int_val} < 0, setting to 0")
            return 0
        
        if clamp_to_uint8:
            # Clamp to UInt8 range (0-255) for existing tables
            if int_val > 255:
                print(f"⚠️ Warning: {field_name} value {int_val} > 255, clamping to 255 for UInt8 table")
                return 255
        else:
            # Clamp to UInt64 range (0-18446744073709551615) for new tables
            if int_val > 18446744073709551615:
                print(f"⚠️ Warning: {field_name} value {int_val} > 18446744073709551615, clamping to 18446744073709551615")
                return 18446744073709551615
        
        return int_val
    except (ValueError, TypeError):
        print(f"⚠️ Warning: Invalid {field_name} value {value}, setting to 0")
        return 0
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count >= max_retries:
                raise
            print(f"⚠️ Landing table batch insertion failed, retrying ({retry_count}/{max_retries}): {e}")
            time.sleep(1)
    return False
...
def push_to_landing_table(connection_params, landing_records, batch_size=100000):
    ...
    if not landing_records:
        print("⚠️ No landing records to push")
        return 0
...
def _setup_schema_and_optimize(client, extractor):
    ...
    print("✅ ClickHouse schema setup and optimization completed")
...
def _collect_device_info_map(extractor):
    ...
    device_info_map = {}
    measurements = extractor.get('measurements', []) if isinstance(extractor, dict) else extractor.data_store.get('measurements', [])
    ...
    for i in range(0, len(device_info_data), batch_size):
        ...
        if batch:
            try:
                ...
                print(f"⚠️ Warning: Error inserting device info batch: {e}")
...
def _push_landing_records_if_available(connection_params, extractor, batch_size):
    ...
    data_store = extractor if isinstance(extractor, dict) else extractor.data_store
    
    if 'landing_records' in data_store and data_store['landing_records']:
        landing_start = time.time()
        print(f"🚀 Solution 5: Pushing {len(data_store['landing_records'])} landing table records...")
        
        landing_pushed = push_to_landing_table(
            connection_params, 
            data_store['landing_records'], 
            batch_size
        )
        ...
        landing_count = len(data_store.get('landing_records', []))
        print(f"ℹ️ Solution 5: No landing records to push (enhanced parser not used or no records, count: {landing_count})")


def push_to_clickhouse(extractor, host='localhost', port=8123, database='default', 
    ...
    Parameters:
    - extractor: StdfDataExtractor instance with processed data OR measurement data dict
    - host: ClickHouse server hostname
    ...
    try:
        ...
        with ClickHouseConnectConnectionManager(setup_pool) as client:
        ...
        measurements = extractor.get('measurements', []) if isinstance(extractor, dict) else extractor.data_store.get('measurements', [])
        push_measurements_clickhouse(connection_params, measurements, batch_size)
...
def _execute_batch_with_retry(connection_pool, batch_data, max_retries=3):
    ...
    while retry_count < max_retries:
        try:
            ...
            if retry_count >= max_retries:
                raise
            print(f"⚠️ Batch execution failed, retrying ({retry_count}/{max_retries}): {e}")
            time.sleep(1)
    return False
...
def optimize_table_for_batch_loading(client, device_ids=None):
    ...
    try:
        ...
        client.execute("OPTIMIZE TABLE measurements FINAL")
        
        print("✅ ClickHouse table optimization complete")
        return True
