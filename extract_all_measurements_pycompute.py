...
class MeasurementExtractorPyCompute:
    ...
    def print_sample_measurements(self, count=5):
        ...
        for i, measurement in enumerate(self.measurements[:count]):
            ...
            print(f"  TEST_NUM:     {measurement['TEST_NUM']}")
            print(f"  DEVICE_DMC:   {measurement['WLD_DEVICE_DMC']}")
            print(f"  BIN_CODE:     {measurement['WLD_BIN_CODE']}")
            print(f"  BIN_DESC:     {measurement['WLD_BIN_DESC']}")
            print(f"  PIXEL_X,Y:    ({measurement['WP_POS_X']}, {measurement['WP_POS_Y']})")
            print(f"  FACILITY:     {measurement['WFI_FACILITY']}")
            print(f"  OPERATION:    {measurement['WFI_OPERATION']}")
